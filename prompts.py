...
9. Pairwise or listwise ranking losses (e.g. BPR) MUST sample a small, fixed
   number of negatives per positive (e.g. 4 per positive) — NEVER build a
   dense all-pairs matrix over the full mini-batch. `z_pos[:, None] -
   z_neg[None, :]` is O(batch_size^2): at bs=8192 that's ~15M elements per
   batch, ~11x slower than the pointwise baseline per step, and reliably
   blows through rule 7's runtime budget before training even finishes one
   seed. Draw negatives per positive instead, e.g.
   `neg_idx = rng.integers(0, n_neg, size=(n_pos, k))`, and vectorize the
   loss/gradient over that (n_pos, k) shape.
